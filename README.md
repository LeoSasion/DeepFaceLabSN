...
- 后台分析在本地服务重启后会标记为“已中断”，不会伪装成成功；可取消任务只在右下角唯一进度卡中显示取消按钮。
- “设置 → 导出诊断摘要”只生成本机 JSON 状态摘要，不包含绝对路径、命令参数或终端内容，便于排障前先检查将要分享的信息。
...
- 视频更换使用事务式素材归档，磁盘写入保留 5 GB 安全余量；恢复操作不会静默覆盖当前素材。
- 诊断摘要默认排除绝对路径、命令参数和终端日志；对外发送前仍应由用户自行复核内容。
